import numpy as np
from TMA import Tridiagonal_solver
import scipy
    ...
    for _ in range(M-1):
        ...
        while True:
            ...
            for i in range(N):
                ...
                error[i] = abs(u_iter_old[i]-u_iter[i])
                # error[i] = abs(u_iter_old[i]-u_iter[i])/abs(u_iter_old[i])
            ITER_NUM += 1

            # Loop exit
            # print(max(error))
            # if max(error) < epsilon:
            # if ITER_NUM == 50 or max(error) < epsilon:
            # if max(error) < 0.1:
                # break
            # elif ITER_NUM >= 50:
            if ITER_NUM >= 50:
                raise Exception(
                    f"Accuracy not achieved after 100 iterations. Error {max(error)}")
